...
module hboson
  ...
  real(dp) :: mh2 ! Higgs mass squared 
...
! See eq. (A.15) in Baur and Glover, Nucl.Phys. B339 (1990) 38-66 
  ...
  function ggxgH_hel1amp_fermions(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
    ...
    res = q2 * four * sqrt(two * s * t * u) * &
          & (-four * (one / (u * t) + one / (u * u1) + one / (t * t1)) &
          & - four * ((two * s + t) * BI(k) / u1**2 + (two * s + u) * BI(j) / t1**2) / s &
          & - (s - four * q2) * (s1 * CI(i1) + (u - s) * CI(j1) + (t - s) * CI(k1)) / (s * t * u) &
          & - 8._dp * q2 * (CI(j1) / (t * t1) + CI(k1) / (u * u1))&
          & + half * (s - four * q2) * (s * t * DI(k) + u * s * DI(j) - u * t * DI(i)) / (s * t * u) &
          & + four * q2 * DI(i) / s &
          & - two * (u * CI(k) + t * CI(j) + u1 * CI(k1) + t1 * CI(j1) - u * t * DI(i)) / s**2)

  end function ggxgH_hel1amp_fermions

!=======================================================================================
! The exact ++- helicity amplitude for gg -> gH 
! See eq. (A.16) in Baur and Glover, Nucl.Phys. B339 (1990) 38-66 

  function ggxgH_hel2amp_fermions(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
    ...
    res = q2 * four * sqrt(two * s * t * u) * (four * m2 &
          & + (m2 - four * q2) * (s1 * CI(4) + t1 * CI(5) + u1 * CI(6)) &
          & - half * (m2 - four * q2) * (s * t * DI(3) + u * s * DI(2) + u * t * DI(1))) / (s * t * u)

  end function ggxgH_hel2amp_fermions

!=======================================================================================
! The exact amplitude for qqbar -> gH 
! See eq. (A.19) in Baur and Glover, Nucl.Phys. B339 (1990) 38-66 

  function qqbarxgH_amp_fermions(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
    ...
    res = q2 * (dcmplx(two) + dcmplx(two * s / (s - m2)) * BI(i) &
          &     + dcmplx(four * q2 - u - t) * CI(k))
...
! See eq. (A.15) in Baur and Glover, Nucl.Phys. B339 (1990) 38-66 
  ...
  function ggxgH_hel1amp_fermions_sml(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
  ...
  end function ggxgH_hel1amp_fermions_sml

!=======================================================================================
! The ++- helicity amplitude for gg -> gH in the small mass limit for the loops 
! See eq. (A.16) in Baur and Glover, Nucl.Phys. B339 (1990) 38-66 

  function ggxgH_hel2amp_fermions_sml(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
  ...
  end function ggxgH_hel2amp_fermions_sml

!=======================================================================================
! The amplitude for qqbar -> gH in the small mass limit for the loops 
! See eq. (A.19) in Baur and Glover, Nucl.Phys. B339 (1990) 38-66 

  function qqbarxgH_amp_fermions_sml(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
...
! with scalars instead of fermions in the loop 
  ...
  function ggxgH_hel1amp_scalars(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
    ...
    res = q2 * sqrt(two * s * t * u) * &
          & (four * (one / (u * t) + one / (u * u1) + one / (t * t1)) &
          & + four * ((two * s + t) * BI(k) / u1**2 + (two * s + u) * BI(j) / t1**2) / s &
          & - four * q2 * (s1 * CI(i1) + (u - s) * CI(j1) + (t - s) * CI(k1)) / (s * t * u) &
          & + 8._dp * q2 * (CI(j1) / (t * t1) + CI(k1) / (u * u1)) &
          & + two * (u * CI(k) + t * CI(j) + u1 * CI(k1) + t1 * CI(j1) - u * t * DI(i)) / s**2 & 
          & - four * q2 * DI(i) / s &
          & + two * q2 * (s * t * DI(k) + u * s * DI(j) - u * t * DI(i)) / (s * t * u) )

  end function ggxgH_hel1amp_scalars

!=======================================================================================
! The exact ++- helicity amplitude for gg -> gH 
! with scalars instead of fermions in the loop 

  function ggxgH_hel2amp_scalars(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
    ...
    res = q2 * sqrt(two * s * t * u) * (-four * m2 & 
          & + four * q2 * (s1 * CI(4) + t1 * CI(5) + u1 * CI(6)) &
          & - two * q2 * (s * t * DI(3) + u * s * DI(2) + u * t * DI(1))) / (s * t * u)

  end function ggxgH_hel2amp_scalars

!=======================================================================================
! The exact amplitude for qqbar -> gH 
! with scalars instead of fermions in the loop 

  function qqbarxgH_amp_scalars(s, t, u, m2, q2, i, j, k, i1, j1, k1, BI, CI, DI) result(res)
    ...
    res = -q2 * half * (dcmplx(one) + dcmplx(s / (s - m2)) * BI(i) &
          &             + dcmplx(two * q2) * CI(k))
...
! Modified code from Herwig 6.521 
  ...
  subroutine hboson_Msquared(s, t, u, wtqq, wtqg, wtgq, wtgg, iloop_in, nmass, mass_in, yukawa_in)

    integer, intent(in) :: nmass
    integer, intent(in) :: iloop_in(nmass)
    real(dp), intent(in) :: mass_in(nmass), yukawa_in(nmass)
    ...
    do imass = 1, nmass
      ...
      if (iloop == 0) then 
        ...
        amp(1) = ggxgH_hel1amp_fermions_sml(s, t, u, mh2, mq2, 1, 2, 3, 4, 5, 6, BI, CI, DI)
        amp(2) = ggxgH_hel2amp_fermions_sml(s, t, u, mh2, mq2, 1, 2, 3, 0, 0, 0, BI, CI, DI)
        amp(3) = ggxgH_hel1amp_fermions_sml(t, s, u, mh2, mq2, 2, 1, 3, 5, 4, 6, BI, CI, DI)
        amp(4) = ggxgH_hel1amp_fermions_sml(u, t, s, mh2, mq2, 3, 2, 1, 6, 5, 4, BI, CI, DI)
        ...
        amp(1) = ggxgH_hel1amp_fermions(s, t, u, mh2, mq2, 1, 2, 3, 4, 5, 6, BI, CI, DI)
        amp(2) = ggxgH_hel2amp_fermions(s, t, u, mh2, mq2, 1, 2, 3, 0, 0, 0, BI, CI, DI)
        amp(3) = ggxgH_hel1amp_fermions(t, s, u, mh2, mq2, 2, 1, 3, 5, 4, 6, BI, CI, DI)
        amp(4) = ggxgH_hel1amp_fermions(u, t, s, mh2, mq2, 3, 2, 1, 6, 5, 4, BI, CI, DI)
        ...
        amp(1) = ggxgH_hel1amp_scalars(s, t, u, mh2, mq2, 1, 2, 3, 4, 5, 6, BI, CI, DI)
        amp(2) = ggxgH_hel2amp_scalars(s, t, u, mh2, mq2, 1, 2, 3, 0, 0, 0, BI, CI, DI)
        amp(3) = ggxgH_hel1amp_scalars(t, s, u, mh2, mq2, 2, 1, 3, 5, 4, 6, BI, CI, DI)
        amp(4) = ggxgH_hel1amp_scalars(u, t, s, mh2, mq2, 3, 2, 1, 6, 5, 4, BI, CI, DI)
  ...
  function born_ampsquare(iloop_in, mass_in, yukawa_in) result(res)
    ...
    do i = 1, size(iloop_in)
      ...
      select case(iloop_in(i))
       ...
       !case default
          !call wae_error("born_ampsquare: unknown iloop")
      end select
