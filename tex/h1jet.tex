...
After the Higgs discovery in 2012, one of most urgent tasks of the
Large Hadron Collider (LHC) is the characterisation of the Higgs
...
form of signal strenght or cross section measurements, offer powerful
constraints on Higgs anomalous couplings.
...
to give exactly the same amplitude for Higgs production as the SM.
...
observation of top quarks in association with the Higgs. The other is
to break the top loop by looking at Higgs production at large
transverse momentum, where the Higgs recoils against a hard jet. Both
are indirect probes of new physics effects. The latter is more
difficult experimentally, in that it relies on appreciating small
deviation from the SM in the shape of the Higgs transverse momentum
distribution, in a region where the phase space closes. It
nevertheless can give a direct access to new physics coupling gluons
to Higgs through loops.

Higgs sector aside, production of colour singlets at high transverse
momentum is commonly used as a probe of new physics. Another relevant
example is the production of monojets, which can either recoiling
either against dark matter, or against a SM $Z$ boson decaying into
neutrinos.
...
with Monte Carlo event generators. These codes, although very general,
have the drawback of being quite slow. Also, accessing interference
terms between new physics and the SM, which carry information on the
strenght of new interactions, are difficult to extract from Monte
Carlo integrators because they are not positive definite. The aim of
this paper is to describe a method to obtain the transverse momentum
spectrum of a colour singlet in a second or less, and its concrete
implementation in the program H1jet. This program makes it possible to
predict the effects of several models in a short amount of time, and
then to devise a more refined cut-based search strategy only for the
one showing the largest deviations with respect to the SM.

More precisely, H1jet predicts the transverse momentum distribution of
...
briefly describe the method underlying H1jet. In
section~\ref{sec:manual} we describe in detail how H1jet works in
practice. In particular, we show how it can be installed and run, and
present the features currently implemented. In
section~\ref{sec:benchmarking} we present a detailed comparison with
the existing program SusHi for Higgs production both in the SM and
beyond. In section~\ref{sec:newprocs} we explain how a user can
implement a model of new physics inside H1jet. We chose ALP
production, giving rise to a monojet. We then go all the way from the
generation of the Feynman rules with Feynrules, to the calculation of
the amplitude with Feyncalc and its subsequent interface with H1jet to
obtain the ALP transverse momentum spectrum. Last,
section~\ref{sec:the-end} presents our conclusions.
...
H1jet will print out a brief summary of the settings and parameters used, as well as the Born cross section $\sigma_0$, followed by the $\mathrm{d}\sigma/\mathrm{d}p_{T}$ and $\sigma(p_{T})$ for each $p_T$ bin. \\ 
...
\section{Adding new processes to H1jet}
\label{sec:newprocs} 
...
\end{mmaCell}
For a $2 \rightarrow 1$ process, the hadronic cross section is given in \autoref{eq:Born-had}, where the partonic luminosity $\mathcal{L}_{gg}$ is handled by H1jet. Hence, we need to multiply our squared matrix element with:
\begin{mmaCell}{Code}
...
the program is a feature that is not available in SusHi.
