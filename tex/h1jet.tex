...
\section{The method}
\label{sec:method}
Before explaining how H1jet works, we consider first the Born cross
section for producing a colour singlet $X$, e.g.\ a Higgs, of mass
$m_X$. This will allow us to set the notation for the rest of the
...
\end{equation}
There can be various partonic subprocesses that can contribute to $X$
production. Let us denote with $M_{ij}$ (with
$i,j=g,q_f,\bar q_{\bar f}$) the amplitude for the subprocess
$ij\to X$ (e.g. $q_f \bar q_f \to Z$). The Born partonic cross section for
this subprocess is
\begin{equation}
  \label{eq:Born-xsct}
  \hat\sigma_{ij}=\frac{1}{2\hat s} |M_{ij}(m_X^2)|^2(2\pi) \delta\left(\hat s-m_X^2\right)=\frac{\pi}{m_X^2} |M_{ij}(m_X^2)|^2 \delta\left(\hat s-m_X^2\right)\,.
\end{equation}
The corresponding hadronic cross section is given by
\begin{equation}
  \label{eq:Born-had}
  \begin{split}
  \sigma& =\sum_{i,j} \int_0^1 dx_1\, f_{i/p}(x_1,\mu_F)\int_0^1 dx_2 \, f_{i/p}(x_1,\mu_F)\sum_{i,j}
\frac{\pi}{m_X^2} |M_{ij}(m_X^2)|^2 \delta\left(x_1 x_2 s-m_X^2\right)\\
...
$\mathcal{L}_{ij}\left(m_X^2/s,\mu_F\right)$ from somewhere, we are
able to obtain a numerical prediction for the cross section from a
simple multiplication. There are indeed numerical tools that are able
to compute, tabulate and interpolate luminosities with incredible efficiency, for instance the
program HOPPET. Through a simple interface with HOPPET, we are able to
compute the Born cross-section given $M_{ij}$. This procedure is the
same adopted in the program JetVHeto computing cross sections for
colour singlets with a veto on additional jets.
...
incoming channel as defined in eq.~\eqref{eq:hoppet-lumi}
\input{manual}
