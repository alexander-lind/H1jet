...
\begin{lstlisting}
	nmax: 2 
\end{lstlisting}
This should be followed by \texttt{nqmax} number of lines -- one for each particle loop -- in the format of four numbers specifying the mass, $\kappa$, $\tilde{\kappa}$, and loop approximation (see later), in that order. For example, for a SM top quark: 
\begin{lstlisting}
	173.5 1.0 0.0 1 
\end{lstlisting}
...
\begin{equation}
	\mathcal{L} \supset -m_q \bar{q}q - \kappa \frac{m_T}{v} \bar{q}q h + i \tilde{\kappa} \frac{m_q}{v} \bar{q} \gamma_5 q h\,,
\end{equation}
